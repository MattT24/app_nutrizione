...
| A5.1 | Diritto alla cancellazione (art. 17) completo | ✅ | **FATTO** (Wave 1) + **sistematizzato F-DEL-CASCADE (Wave 2)**: `ClienteService.deleteMyCliente` rimuove l'intero grafo figlio (schede→albero, misurazioni/plicometrie/obiettivi/blacklist/tag via cascade ORM; appuntamenti/TDEE/fascicolo+**file su disco**/attività recenti via `eliminaFigliNonCascade`). Meccanismo **ORM-first ibrido** (niente `ON DELETE CASCADE` DB: H2≠TiDB fino a v8.5 → orfano mascherato). Lacuna non-FK chiusa: `EventoGamification.clienteId` **anonimizzato a NULL**; `AuditLog.clienteId` **preservato** (obbligo legale art. 17(3)(b)). Regression-guard `ClienteDeleteCascadeCompletoIntegrationTest` (un figlio per ogni tipo → zero orfani + le due asserzioni opposte sui `clienteId`). Regola di coverage in CLAUDE.md. **Aperto (tracciato):** semplificazione dual-FK `AlimentoAlternativo`. | FATTO |
...
| A5.3 | Diritto di limitazione (art. 18) | ✅ | **FATTO (Wave 2)**: stato `Cliente.trattamentoLimitato` (+data/motivo, migr. `017`); `LimitazioneTrattamentoValidator.assertNonLimitato` chiamato **dopo** l'ownership in tutti i punti write/produce/send (~50 metodi/16 service) → **`TrattamentoLimitatoException` HTTP 423**. Regola: *blocca ciò che scrive/produce/invia, non ciò che il titolare legge* (export/download consentiti, share bloccato; delete art. 17 non bloccato). Endpoint `PATCH /api/clienti/{id}/limitazione(/revoca)` (`CLIENTE_UPDATE`), atto auditato `LIMITAZIONE_ATTIVATA`/`REVOCATA`. FE: badge/banner/modale + disabilitazioni. Test `LimitazioneTrattamentoIntegrationTest` (incl. **cross-tenant→403 non 423**) + unit. | FATTO. Interpretazione art. 18 **da confermare col legale** (come A6). In prod: eseguire `017_clienti_trattamento_limitato.sql`. |
| A6 | Retention / cancellazione dati clinici (art. 5(1)(e)) | ✅ | **MECCANISMO FATTO + verificato (Wave 2)**: `eliminaClienteCompleto` (cancellazione canonica condivisa art. 17/A6), `ultimoContattoClinico` + query di eleggibilità (12 `NOT EXISTS` sul sotto-albero + floor `createdAt`), `RetentionService` con valvole (dry-run, quarantena+grace, circuit-breaker, rate-limit oldest-first, TOCTOU sotto lock, lock singola-esecuzione), `RetentionScheduler` (04:00), audit `RETENTION_QUARANTENA`/`RETENTION_PURGE`; migrazioni `021`+`022` eseguite su TiDB. Verificato adversarial (query/valvole/purge CLEAN + test mutation-killer F1/F2). Chiude anche **F-USER-DEL** (erasure account completa). | **MECCANISMO FATTO** (Wave 2). ⚠️ `dry-run=true` di default → **purge OFF**: attivare solo dopo la conferma delle **durate col legale/titolare** (provvisorie: 10 anni, art. 2946 c.c.). `accettazioni_documento` all'erasure account = open-legale (`LEGAL-REVIEW §13`). |
...
| A9 | Informativa / DPA alla registrazione | 🟡 | **Meccanismo FATTO (Wave 2)**: 3 accettazioni separate (PRIVACY_POLICY/TERMINI_SERVIZIO/DPA) via `@AssertTrue` su `RegisterRequest`/`GoogleRegisterRequest` + 3 checkbox obbligatorie nei due screen FE (register + complete-google-profile); accettazioni versionate persistite. | Resta Wave 3: i **testi** reali (Informativa/DPA) + il gate FE al login su cambio versione (`/pending`). | Wave 2 fatto / testi PROD |
...
| ID | Requisito | Stato | Evidenza | Decisione | Fase |
|----|-----------|:-----:|----------|-----------|:----:|
| D1 | Rischio qualificazione dispositivo medico (MDR) | 🟡⚠️ | Nessun claim di diagnosi/terapia/cura; motore di alert clinici per-paziente (`ALERT_GRAVE`, tag `PAT_*`) ora con override consapevole + audit `[verificato in locale]` | **Intended use dichiarato dal team**: sono aiuti alla compilazione delle diete che *prescindono* dalla decisione del nutrizionista. Raccomandazione **(b) FATTA** (finding D1): il blocco grave (incl. allergie) è ora **superabile con conferma consapevole esplicita** — flag dedicato `confermaBloccoGrave` — e l'override è **auditato** (`OVERRIDE_ALERT_GRAVE`), così la decisione finale resta formalmente del professionista come per le intolleranze. **Resta (a): formalizzare l'intended use per iscritto** (una pagina). ⚠️ Nota: i percorsi template/duplicazione non passano ancora dal check clinico (finding F-D1a in ROADMAP). | NEXT |
...
- [~] **Accettazione documenti in registrazione** (A4/A9) — meccanismo FATTO (Wave 2: entità versionata, `@AssertTrue`, checkbox FE, endpoint `/api/accettazioni`). Resta il **contenuto legale** (Privacy/Termini/DPA) + gate FE al login (Wave 3). In prod: eseguire `014_accettazioni_documento.sql`.
...
- [x] **Retention/cancellazione** dei dati clinici implementata (A6) — **meccanismo** fatto+verificato (Wave 2); ⚠️ `dry-run=true`/purge OFF finché le **durate** non sono confermate col legale/titolare; migrazioni `021`+`022` eseguite
